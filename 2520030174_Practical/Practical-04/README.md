...
waitpid() is used by the parent process to wait for a particular child process.
waitpid(pid, NULL, 0);
Here, pid is the Process ID (PID) of the child process.

Zombie Process

A zombie process is a child process that has completed execution, but its parent has not yet collected its exit status. Therefore, it stays in the process table.
